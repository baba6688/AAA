...
class DependencyResolver:
    ...
    def analyze_dependencies(self, class_info: Dict[str, Any]) -> List[DependencyNode]:
        ...
        try:
            ...
            self.logger.error(f"分析依赖关系失败 {getattr(class_info, 'class_name', 'unknown')}: {e}")
    ...
    def _analyze_configuration_dependencies(self, cls, class_info) -> List[DependencyNode]:
        """分析配置依赖 - 安全版本"""
        dependencies = []
        
        # 安全处理：ClassInfo 对象没有配置方法
        # 跳过配置依赖分析以避免错误
        try:
            if hasattr(class_info, 'name'):
                class_name = class_info.name
            elif hasattr(class_info, 'get') and callable(getattr(class_info, 'get')):
                class_name = getattr(class_info, 'name', 'unknown')
            else:
                class_name = getattr(class_info, 'name', 'unknown')
                
            self.logger.debug(f"跳过配置依赖分析: {class_name}")
        except Exception:
            self.logger.debug("跳过配置依赖分析: unknown")
    ...
    def _find_candidate_implementations(self, dependency: DependencyNode) -> List[str]:
        ...
        if dependency.type == DependencyType.CONFIGURATION:
            ...
            for class_info in all_classes:
                class_name = getattr(class_info, 'class_name', None)
