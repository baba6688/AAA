...
class AutoWiringFactory:
    ...
    def create_instance(self, 
        ...
        try:
            ...
            if not cls:
                # 如果没有class_object，尝试动态导入
                try:
                    import importlib
                    module_name = getattr(class_info, 'module', None)
                    class_name_val = getattr(class_info, 'name', None)
                    if module_name and class_name_val:
                        module = importlib.import_module(module_name)
                        cls = getattr(module, class_name_val)
                        # 缓存类对象到class_info
                        class_info.class_object = cls
                        self.logger.debug(f"动态导入类: {module_name}.{class_name_val}")
                    else:
                        raise ValueError(f"无法获取类对象: {class_name} - 缺少模块或类名信息")
                except Exception as import_error:
                    self.logger.error(f"动态导入类失败 {class_name}: {import_error}")
                    raise ValueError(f"无法获取类对象: {class_name} - 导入失败: {import_error}")
            if not cls:
    ...
    def _inject_configuration(self, cls: Type, signature: inspect.Signature, class_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for param_name, param in signature.parameters.items():
            ...
            if param_name in ['config', 'configuration', 'settings']:
                ...
                # 安全获取配置节 - 使用空字典作为默认值
                config_section = {}
                # 跳过 get_config_section 调用，ClassInfo 没有此方法
                # 如果有配置需求，可以从其他地方获取
                # 例如: config_section = self.config_manager.get_section(class_name)
                
                if not config_section:
                    # 使用类名作为默认配置章节
                    config_section = cls.__name__.lower()
